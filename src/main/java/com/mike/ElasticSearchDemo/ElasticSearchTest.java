package com.mike.ElasticSearchDemo;
...
public class ElasticSearchTest {
	...
	public void createIndexByJson() throws UnknownHostException {
       try{
		...
		// 2 创建文档
		IndexResponse indexResponse = client.prepareIndex("blog", "article", "1").setSource(json, XContentType.JSON).execute().actionGet();
	...
	public void updateData() throws Throwable {
		...
		updateRequest.doc(XContentFactory.jsonBuilder().startObject()
				...
				.field("content",
						"它提供了一个分布式多用户能力的全文搜索引擎，基于RESTful web接口。前景无限")
	...
	public void wildcardQuery() {
		...
		// 1 通配符查询
		SearchResponse searchResponse = client.prepareSearch("blog").setTypes("article")
				.setQuery(QueryBuilders.wildcardQuery("content", "*无*")).get();
...
//		PutMappingRequest mapping = Requests.putMappingRequest("blog2").type("article").source(builder);
        ...
        IndexResponse response = client.prepareIndex("blog2", "article").setSource(builder).get();
